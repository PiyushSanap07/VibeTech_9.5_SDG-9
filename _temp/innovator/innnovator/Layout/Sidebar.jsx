...
const Sidebar = ({ user }) => {
  ...
  const navItems = [
    ...
    { path: '/funders', icon: Users, label: 'Funders' },
    { path: '/messages', icon: MessageSquare, label: 'Messages' },
    { path: '/proposals', icon: FileText, label: 'Proposals' },
    { path: '/milestones', icon: ListChecks, label: 'Milestones' },
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.path}
              to={item.path}
              className={`relative flex items-center space-x-3 px-4 py-3.5 rounded-2xl font-bold transition-all duration-300 group ${
                isActive 
