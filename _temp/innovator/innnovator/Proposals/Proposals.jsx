...
import ProposalForm from '../Dashboard/ProposalForm';
      ...
      {loading ? (
        ...
        <div className="space-y-6 pb-20">
          {filteredProposals.map((p, i) => (
            ...
            >
              <Link to={`/proposal/${p.id}`} className="block p-8 hover:bg-slate-50/50 transition-all">
                  ...
                  <div className="space-y-4 flex-1">
                    <div className="flex items-center space-x-4">
                      ...
                      <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                        p.status === 'Funded' ? 'bg-emerald-100 text-emerald-600' : 'bg-indigo-100 text-indigo-600'
                      }`}>
                        {p.status}
                      </span>
                      {p.origin === 'Chat' && (
